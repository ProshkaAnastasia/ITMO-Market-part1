...
class OrderService(
    ...
    fun addToCart(userId: Long, productId: Long, quantity: Int): OrderResponse {
        ...
        if (existingItem.isPresent) {
            ...
            val newItem = OrderItem(
                ...
                productId = productId,
                quantity = quantity,
                price = product.price // Price from DTO
            )
    ...
    private fun Order.toResponse(): OrderResponse {
        ...
        val itemResponses = items.map { item ->
            // Decoupled: Get Product DTO from ProductService
            // Optimization note: looping calls to service might be slow. In real apps, consider bulk fetch.
            val productDto = productService.getProductById(item.productId)
            
            OrderItemResponse(
                id = item.id,
                product = productDto, // This DTO already contains ratings/counts
