...
class OrderService(
    ...
    fun addToCartFallback(userId: Long, productId: Long, quantity: Int, t: Throwable): OrderResponse {
        throw ServiceUnavailableException(
            "Cannot add product to cart. Product service is temporarily unavailable."
        )
    }
    ...
    private fun Order.toResponse(): OrderResponse {
        ...
        val itemResponses = items.map { item ->
            try {
                ...
                // Return order item with null product when service unavailable
                OrderItemResponse(
                    id = item.id,
                    product = null,
                    quantity = item.quantity,
